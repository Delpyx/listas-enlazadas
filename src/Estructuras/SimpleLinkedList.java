...
public class SimpleLinkedList<ELEMENT extends Comparable<ELEMENT>> implements ILinkedList<ELEMENT> {
    ...
    public void addFirstNoRepetido(ELEMENT item) {
        if(item == this.head.item) {
            return;
        }
        Node<ELEMENT> temp = new Node<ELEMENT>(item, this.head);
        if (this.count == 0) {
    ...
    public void eliminarPrimeraCoincidencia(ELEMENT item) {
        if (this.head == null) {
            return;
        }
        if (this.head.item.compareTo(item) == 0) {
            removeFirst();
            return;
        }
        Node<ELEMENT> actual = this.head;
        while(actual.next != null && actual.next.item.compareTo(item) != 0) {
            actual = actual.next;
        }
        if(actual.next == null) {
            return;
        }
        Node<ELEMENT> eliminado = actual.next;
