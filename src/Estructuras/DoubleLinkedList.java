...
import Punto1.Libro;
    ...
    public class DoubleLinkedList<ELEMENT extends Comparable<ELEMENT>> implements ILinkedList<ELEMENT> {
        ...
        protected class Node<ELEMENT> {
            protected ELEMENT item; //dato
            protected Node<ELEMENT> next;//apunta para adelante
            protected Node<ELEMENT> prev;//apunta para atras
        ...
        public ELEMENT devolverMenor(){
            if (this.size() <= 0) {
                throw new RuntimeException("La lista está vacía...");
            }
            Node<ELEMENT> actual = this.head;
            ELEMENT menor = actual.item;

            while(actual.next != null){
                if(actual.item.compareTo(menor)<0){
                    menor = actual.item;
                }
                actual = actual.next;
            }
        ...
        public void agregarMenor(ELEMENT item){
            if(this.size() <= 0 || item.compareTo(this.head.item) <= 0){
                addFirst(item);
                return;
            }
            Node<ELEMENT> actual = this.head;
            while (actual != null && item.compareTo(actual.item) > 0) {
                actual = actual.next;
            }
            if (actual == null) {
                this.addLast(item);
                ...
                Node<ELEMENT> nodoNuevo = new Node<>(item);
                Node<ELEMENT> nodoAnterior = actual.prev;
                nodoAnterior.next = nodoNuevo;
                nodoNuevo.prev = nodoAnterior;
                actual.prev = nodoNuevo;
